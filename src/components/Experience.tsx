  ...
  return (
    <section id="experience" className="py-32 px-6 max-w-5xl mx-auto">
      <motion.div
        ...
        className="mb-16"
      >
        <h2 className="text-4xl md:text-5xl font-serif font-light tracking-tighter text-white mb-4">Professional Experience</h2>
        ...
        {resumeData.experience.map((exp, idx) => (
          <motion.div
            ...
            className={cn(
              ...
              expandedIdx === idx 
                ? "bg-white/10 border-white/20 shadow-2xl shadow-accent/5" 
            ...
            >
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-white/40">
                  <span className="flex items-center gap-1.5"><Calendar size={14} className="text-accent/60" /> {exp.dates}</span>
                  {exp.location && <span className="flex items-center gap-1.5"><MapPin size={14} className="text-accent/60" /> {exp.location}</span>}
                </div>
              </div>
              <motion.div
                    ...
                    {exp.bullets.length > 0 ? (
                      <ul className="space-y-4">
                        {exp.bullets.map((bullet, bIdx) => (
                          ...
                          >
                            <Zap size={16} className="mt-1.5 shrink-0 text-accent/40" />
                            <span>{bullet}</span>
                          </motion.li>
                        ))}
                      </ul>
